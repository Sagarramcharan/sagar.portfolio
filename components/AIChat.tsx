    ...
    <div className="fixed bottom-6 right-6 z-[100]">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[350px] md:w-[400px] h-[500px] glass rounded-[2rem] border border-orange-500/20 shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-4 text-sm leading-relaxed ${
                    msg.role === 'user' 
                      ? 'orange-gradient text-white rounded-t-2xl rounded-bl-2xl font-medium' 
      ...
      <motion.button
        ...
        className={`w-16 h-16 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 ${
          isOpen ? 'bg-white text-black' : 'orange-gradient text-white'
        }`}
      >
